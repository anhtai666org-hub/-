...
app.post("/api/auth/register",async(req,res)=>{
  ...
  if(!username||!email||!password||String(password).length<8)return res.status(400).json({error:"Vui lòng nhập đủ thông tin; mật khẩu tối thiểu 8 ký tự."});
  const e=String(email).trim().toLowerCase();
  if([...users.values()].some(u=>u.email===e))return res.status(409).json({error:"Email đã được sử dụng."});
  const u={id:uid(),username:String(username).trim().slice(0,40),email:e,passwordHash:await bcrypt.hash(String(password),12),createdAt:Date.now()};
  users.set(u.id,u);conversations.set(u.id,[]);memories.set(u.id,[]);req.session.userId=u.id;
  res.json({user:pub(u)});
});
app.post("/api/auth/login",async(req,res)=>{
  const u=[...users.values()].find(x=>x.email===String(req.body?.email||"").trim().toLowerCase());
  if(!u||!(await bcrypt.compare(String(req.body?.password||""),u.passwordHash)))return res.status(401).json({error:"Email hoặc mật khẩu không đúng."});
...
app.post("/api/conversations",auth,(req,res)=>{
  const c={id:uid(),title:"Cuộc trò chuyện mới",updatedAt:Date.now(),messages:[]};
  conversations.get(req.user.id).unshift(c);res.json({conversation:c});
});
app.get("/api/conversations/:id",auth,(req,res)=>{
  const c=(conversations.get(req.user.id)||[]).find(x=>x.id===req.params.id);
  if(!c)return res.status(404).json({error:"Không tìm thấy cuộc trò chuyện."});res.json({conversation:c});
});
app.patch("/api/conversations/:id",auth,(req,res)=>{
  const c=(conversations.get(req.user.id)||[]).find(x=>x.id===req.params.id);
  if(!c)return res.status(404).json({error:"Không tìm thấy cuộc trò chuyện."});
  c.title=String(req.body?.title||c.title).trim().slice(0,100);c.updatedAt=Date.now();res.json({conversation:c});
});
app.delete("/api/conversations/:id",auth,(req,res)=>{
  const a=conversations.get(req.user.id)||[],i=a.findIndex(x=>x.id===req.params.id);
  if(i<0)return res.status(404).json({error:"Không tìm thấy cuộc trò chuyện."});a.splice(i,1);res.json({ok:true});
});

app.get("/api/memories",auth,(req,res)=>res.json({memories:memories.get(req.user.id)||[]}));
app.post("/api/memories",auth,(req,res)=>{
  const text=String(req.body?.content||"").trim();if(!text)return res.status(400).json({error:"Memory trống."});
  const m={id:uid(),content:text,createdAt:Date.now()};memories.get(req.user.id).push(m);res.json({memory:m});
});
app.delete("/api/memories/:id",auth,(req,res)=>{
  const a=memories.get(req.user.id)||[],i=a.findIndex(x=>x.id===req.params.id);if(i>=0)a.splice(i,1);res.json({ok:true});
});
...
app.post("/api/chat",auth,async(req,res)=>{
  ...
  if(text.length>12000)return res.status(400).json({error:"Tin nhắn quá dài."});
  c.messages.push({role:"user",content:text,createdAt:Date.now()});
  if(c.title==="Cuộc trò chuyện mới")c.title=text.slice(0,55);c.updatedAt=Date.now();
  const mem=memories.get(req.user.id)||[];
  ...
  try{
    ...
    c.messages.pop();res.status(502).json({error:"Không gọi được AI lúc này."});
  }
});
app.use(express.static("public"));
app.listen(PORT,"0.0.0.0",()=>console.log(`YamadaNihon AI listening on ${PORT}`));